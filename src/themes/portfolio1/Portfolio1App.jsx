...
import { Home, User, Briefcase, FolderKanban, GraduationCap, Send } from "lucide-react";
...
export default function Portfolio1App() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const shouldShow =
        currentScrollY > lastScrollY && currentScrollY > window.innerHeight * 0.5;
      const isTop = currentScrollY < window.innerHeight * 0.5;
      if (shouldShow) setShowDock(true);
      else if (isTop) setShowDock(false);
        ...
        <AnimatePresence>
          {showDock && (
            <motion.div
              ...
              className="fixed bottom-2 left-0 right-0 z-[998] hidden md:block"
            >
              <Dock
                items={dockItems}
                panelHeight={56}
